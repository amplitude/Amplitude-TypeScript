...
try {
  ...
  const decision = report.settings.elementSelectorDecision;
  console.log(`\nElement selector: ${decision.shouldEnable ? 'enable' : 'leave disabled'}`);
  decision.reasons.forEach((reason) => console.log(`- ${reason}`));
  if (decision.shouldEnable) {
    const { autogeneratedIdPatterns, unstableClassPatterns } = decision.additionalRegex;
    console.log('Additional remote-config regex:');
    if (!autogeneratedIdPatterns.length && !unstableClassPatterns.length) {
      console.log('- none');
    }
    autogeneratedIdPatterns.forEach((pattern) => console.log(`- autogeneratedIdPatterns: ${pattern}`));
    unstableClassPatterns.forEach((pattern) => console.log(`- unstableClassPatterns: ${pattern}`));
  }

  const changed = report.settings.requiredChanges;
  ...
  changed.forEach((change) => {
    if (change.kind === 'list') {
      console.log(`- ${change.field}: ${change.entryCount.sdkDefault} -> ${change.entryCount.recommended} entries`);
