...
describe('EventCompressor', () => {
  ...
  describe('zlib replay event encoding (default)', () => {
    ...
    test('full snapshot drains queued events before snapshot', async () => {
      const zlibConfig = new SessionReplayLocalConfig('static_key', {
        loggerProvider: mockLoggerProvider,
        sampleRate: 1,
        performanceConfig: { enabled: false, legacyReplayEventEncoding: false },
    ...
    test('worker delivers compressed payloads', async () => {
      const zlibConfig = new SessionReplayLocalConfig('static_key', {
        loggerProvider: mockLoggerProvider,
        sampleRate: 1,
        performanceConfig: { enabled: false, legacyReplayEventEncoding: false },
      ...
      expect(mockWorker.postMessage).toHaveBeenCalledWith(
        expect.objectContaining({ gzipReplayEvents: true, sessionId }),
      );

      mockWorker.onmessage?.({ data: { compressedEvent: '"wrapped"', sessionId } });
      expect(addEventMock).toHaveBeenCalled();
    });

    test('addCompressedEvent logs unexpected worker postMessage errors', () => {
      const zlibConfig = new SessionReplayLocalConfig('static_key', {
        loggerProvider: mockLoggerProvider,
        sampleRate: 1,
        performanceConfig: { enabled: false, legacyReplayEventEncoding: false },
    ...
    test('addCompressedEvent retries JSON on DataCloneError', () => {
      const zlibConfig = new SessionReplayLocalConfig('static_key', {
        loggerProvider: mockLoggerProvider,
        sampleRate: 1,
        performanceConfig: { enabled: false, legacyReplayEventEncoding: false },
    ...
    test('calls onFullSnapshotProcessed on zlib full snapshot path', async () => {
      const zlibConfig = new SessionReplayLocalConfig('static_key', {
        loggerProvider: mockLoggerProvider,
        sampleRate: 1,
        performanceConfig: { enabled: false, legacyReplayEventEncoding: false },
