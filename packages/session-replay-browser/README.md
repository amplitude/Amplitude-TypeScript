...
|`performanceConfig.legacyReplayEventEncoding`|`boolean`|No|`false`|When `true`, stores replay events as plain key-ordered JSON strings. By default each event is zlib-compressed (`CompressionStream('deflate')`) and wrapped like legacy `pack()`. Track POST bodies are always uncompressed JSON. Uses the compression web worker when available.|
