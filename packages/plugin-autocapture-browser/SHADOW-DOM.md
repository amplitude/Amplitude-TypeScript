...
- [ ] **Micro-benchmark / perf script** — DOM churn on `test-server/shadow-dom-perf.html` with long-task measurement (`e2e/shadow-dom-perf.spec.ts`)
...
| `test/helpers.test.ts`, `test/hierarchy.test.ts`, `test/observables.test.ts` | Capture helpers, ancestor walks, mutation fan-out |
| `e2e/shadow-dom.spec.ts` | Real browser: retargeting, `composedPath` |
| `e2e/shadow-dom-perf.spec.ts` | Differential TBT / long-task guard (flat, nested, app-shell churn) |
