...
function getSharedSelectorEngine(): SelectorEngine {
  if (!sharedSelectorEngine) {
    sharedSelectorEngine = createSelectorEngine(resolveSelectorConfig(), { logger: sharedSelectorLogger });
  }
...
export class DataExtractor {
  ...
  getElementPath = (element: Element | null): string => {
    ...
    const config = this.selectorEngine.getConfig();
    const elementPath = this.selectorEngine.generate(element);

    if (config.enabled) {
      sharedSelectorLogger?.debug('@amplitude/plugin-autocapture-browser: element-selector engine enabled', {
        elementPath,
  ...
  updateSelectorConfig = (remote?: ElementSelectorRemoteConfig | null, logger?: ElementSelectorLogger): void => {
    if (logger) {
      sharedSelectorLogger = logger;
    }

    const resolved = resolveSelectorConfig(remote ?? undefined, logger);
    const previousEnabled = sharedSelectorEngine?.getConfig().enabled;

    if (!sharedSelectorEngine) {
      sharedSelectorEngine = createSelectorEngine(resolved, { logger: sharedSelectorLogger });
    } else {
      sharedSelectorEngine.updateConfig(resolved);
    }

    logger?.debug('@amplitude/plugin-autocapture-browser: element-selector config updated', {
      enabled: resolved.enabled,
      wasEnabled: previousEnabled ?? false,
      toggled: (previousEnabled ?? false) !== resolved.enabled,
      explicitTrackingAttribute: resolved.explicitTrackingAttribute,
      autogeneratedIdPatternCount: resolved.autogeneratedIdPatterns.length,
      unstableClassPatternCount: resolved.unstableClassPatterns.length,
      maxAncestorWalkDepth: resolved.maxAncestorWalkDepth,
    });
  };
