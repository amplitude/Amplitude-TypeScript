...
export class DataExtractor {
  ...
  getElementPath = (element: Element | null): string => {
    ...
    if (config.enabled && sharedSelectorLogger) {
  ...
  updateSelectorConfig = (remote?: ElementSelectorRemoteConfig | null, logger?: ElementSelectorLogger): void => {
    ...
    const engine = getSharedSelectorEngine();
    const previousEnabled = engine.getConfig().enabled;
    ...
    logger?.debug('@amplitude/plugin-autocapture-browser: element-selector config updated', {
      ...
      toggled: previousEnabled !== resolved.enabled,
