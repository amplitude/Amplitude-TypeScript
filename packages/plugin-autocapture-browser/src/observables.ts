...
import { SHADOW_OFF, type ShadowGate, type ShadowMode } from './shadow-mode';
...
/** Read a possibly-absent gate. No gate means the off path. */
const readGate = (gate?: ShadowGate): ShadowMode => gate?.get() ?? SHADOW_OFF;
...
export const createMutationObservable = (shadowGate?: ShadowGate): Observable<MutationRecord[]> => {
  return new Observable<MutationRecord[]>((observer) => {
    ...
    //
    // `?? 0` is unreachable: a mutation only reaches this callback from a root
    // passed to `observeRoot`, which records the depth of every ShadowRoot. It
    // stands in for a non-null assertion.
...
export const createExposureObservable = (
  ...
  return new Observable<Event>((observer) => {
    ...
    // Registers every element matching the allowlist under `root`, descending
    // into open shadow roots when the mode is enabled.
    //
