...
/**
 * Like `element.closest(selector)`, but able to cross OPEN shadow boundaries:
 * when no ancestor in the element's own tree matches, the walk hops to the
 * shadow host and retries `closest` from there, up to `maxCrossings` boundary
 * hops. With `maxCrossings = 0` this is exactly `element.closest` — the off
 * path is unchanged. Used where a single-selector `closest` needs to see
 * light-DOM ancestors of a shadow tree (labeled-event matching, text masking).
 */
export const closestCrossShadow = (element: Element | null, selector: string, maxCrossings = 0): Element | null => {
  let current: Element | null = element;
  let crossings = 0;
  while (current) {
    ...
    if (crossings >= maxCrossings || typeof current.getRootNode !== 'function') {
      return null;
    }
    const host = composedParent(topOfTree(current));
    if (!host) {
      return null;
    }
    crossings += 1;
...
/** The highest ancestor of `element` within its own tree (no boundary crossing). */
const topOfTree = (element: Element): Element => {
  let top = element;
