...
function isCaptureRuleMatch(rule: NetworkCaptureRule, hostname: string, status?: number) {
  ...
  if (status || status === 0) {
    let statusCodeRange = rule.statusCodeRange || DEFAULT_STATUS_CODE_RANGE;
...
export function shouldTrackNetworkEvent(
  networkEvent: NetworkRequestEvent,
  ...
  if (!options.captureRules && networkEvent.status &&
