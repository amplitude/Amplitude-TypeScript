...
describe('createMutationObservable — shadow fan-out', () => {
  ...
  it('falls back to depth 0 when an observed shadow root is missing from the depth map', async () => {
    document.body.innerHTML = `<outer-host></outer-host>`;
    const outer = document.querySelector('outer-host') as Element;
    const outerRoot = attachOpen(outer, `<div id="slot"></div>`);
    ...
    try {
      // Attach shadow before appending so the MO callback (which runs after this
      // synchronous turn) discovers the nested root via collectOpenShadowRoots.
      const nestedHost = document.createElement('nested-host');
      const nestedRoot = attachOpen(nestedHost, `<div id="inner"></div>`);
      outerRoot.getElementById('slot')?.appendChild(nestedHost);
      await tick();

      nestedRoot.getElementById('inner')?.appendChild(document.createElement('span'));
      await tick();

      const sawNested = batches.some((b) => b.some((rec) => rec.target.getRootNode() === nestedRoot));
