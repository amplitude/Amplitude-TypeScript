...
import { createShadowGate, SHADOW_OFF, type ShadowGate, type ShadowMode } from '../src/shadow-mode';
...
/** An enabled shadow mode with the given crossing budget. */
const on = (maxDepth: number): ShadowMode => ({ enabled: true, maxDepth });

/** A gate already armed at the given budget (the post-remote-config steady state). */
const armedGate = (maxDepth: number): ShadowGate => {
  const gate = createShadowGate();
  gate.arm(on(maxDepth));
  return gate;
};

/** A gate that never arms — the default off path. */
...
describe('getClosestElement — shadow crossing', () => {
  ...
  it('respects the crossing budget (one boundary short of the match)', () => {
    // Two nested shadow trees between the target and #app: a budget of 1 can
    // reach the outer host's tree but not all the way out to the light DOM.
    ...
    const buttonRoot = attachOpen(cardRoot.querySelector('my-button') as Element, `<button id="deep">x</button>`);
  ...
  it('still matches within the same tree without crossing', () => {
    expect(getClosestElement(inner, ['#cta'], SHADOW_OFF)).toBe(inner);
  });
});
...
describe('getAncestors — shadow crossing', () => {
  ...
  it('walks through the host into the light DOM when enabled', () => {
    const ancestors = getAncestors(inner, on(1));
...
describe('querySelectorAllDeep', () => {
  ...
  it('returns only light-DOM matches when shadow is off', () => {
...
describe('shadow helpers — never throw on hostile input', () => {
  it('querySelectorAllDeep returns light-DOM matches even if a shadow root query is malformed', () => {
    ...
    expect(querySelectorAllDeep(document, '.track', on(1)).map((el) => el.textContent)).toEqual(['x']);
  });
  ...
  it('getClosestElement does not throw when the start node lacks getRootNode (e.g. window)', () => {
    ...
    expect(() => getClosestElement(exotic, ['button'], on(2))).not.toThrow();
    expect(getClosestElement(exotic, ['button'], on(2))).toBeNull();
  });

  it('querySelectorAllDeep returns [] for an empty selector string', () => {
    document.body.innerHTML = `<button class="track">x</button>`;
    expect(querySelectorAllDeep(document, '', on(1))).toEqual([]);
  });

  it('querySelectorAllDeep defaults to light-DOM only (no mode arg) and accepts an Element root', () => {
    ...
    // Default mode (arg omitted) → light DOM only.
    expect(querySelectorAllDeep(scope, '.track').map((el) => el.textContent)).toEqual(['x']);
    // Element (non-Document) root with piercing on → reaches the shadow root within scope.
    expect(
      querySelectorAllDeep(scope, '.track', on(1))
        .map((el) => el.textContent)
        .sort(),
    ).toEqual(['x', 'y']);
  });

  it('collectOpenShadowRoots is bounded by MAX_SHADOW_DOM_TRAVERSAL_NODES', () => {
    expect(MAX_SHADOW_DOM_TRAVERSAL_NODES).toBeGreaterThan(0);
...
describe('createMutationObservable — shadow fan-out', () => {
  ...
  it('observes mutations in a NESTED shadow root (depth 2) when the budget allows', async () => {
    ...
    const batches: MutationRecord[][] = [];
    const sub = createMutationObservable(armedGate(2)).subscribe((m) => batches.push(m));
  ...
  it('still emits light-DOM mutations even if the shadow gate throws', async () => {
    document.body.innerHTML = `<div id="light"></div>`;
    const batches: MutationRecord[][] = [];
    // A hostile gate that throws must not break event flow (observer.next runs first).
