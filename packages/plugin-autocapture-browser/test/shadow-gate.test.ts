 ...
 */
import { MASKED_TEXT_VALUE, TEXT_MASK_ATTRIBUTE, type IDiagnosticsClient } from '@amplitude/analytics-core';
...
describe('late remote config — exposure discovery picks up pre-existing shadow elements', () => {
  ...
  it('stops running the arm scan once unsubscribed', () => {
    document.body.innerHTML = `<my-host></my-host>`;
    const root = attachOpen(document.querySelector('my-host') as Element, `<button class="track-me">shadow</button>`);
    const shadowButton = root.querySelector('button') as Element;
    ...
    const gate = createShadowGate();
    const obs = createExposureObservable(mutationObservable, ['.track-me'], gate);
    obs.subscribe(() => undefined).unsubscribe();
...
describe('text masking across the shadow boundary', () => {
  /**
   * Regression: `Element.closest` cannot see past a shadow root, so a
   * `data-amp-mask` on a light-DOM ancestor did not mask text of an element
   * captured inside a shadow tree. Harmless while piercing was off (the capture
   * layer never handed `getText` an in-shadow element), a privacy hole once on.
   */
  const buildMaskedShadow = () => {
    ...
    const root = attachOpen(
      ...
      `<span id="secret">super secret value</span>`,
    );
    return root.getElementById('secret') as Element;
  };

  it('masks an in-shadow element when a light-DOM ancestor is marked and piercing is on', () => {
    const extractor = new DataExtractor({});
    extractor.updateSelectorConfig({ shadowDomEnabled: true, maxShadowDomDepth: 2 });
    expect(extractor.getText(buildMaskedShadow())).toBe(MASKED_TEXT_VALUE);
  });

  it('still masks via closest() within a single tree when piercing is off', () => {
    document.body.innerHTML = `<div ${TEXT_MASK_ATTRIBUTE}><span id="secret">super secret value</span></div>`;
    const extractor = new DataExtractor({});
    expect(extractor.getText(document.getElementById('secret') as Element)).toBe(MASKED_TEXT_VALUE);
  });

  it('does not mask an unmarked in-shadow element when piercing is on', () => {
    document.body.innerHTML = `<div><my-host></my-host></div>`;
    const root = attachOpen(document.querySelector('my-host') as Element, `<span id="plain">hello</span>`);
    const extractor = new DataExtractor({});
    extractor.updateSelectorConfig({ shadowDomEnabled: true, maxShadowDomDepth: 2 });
    expect(extractor.getText(root.getElementById('plain') as Element)).toBe('hello');
  });
});
