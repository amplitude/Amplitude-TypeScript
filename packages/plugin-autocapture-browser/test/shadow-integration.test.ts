...
import { createShadowGate, resetSharedShadowGateForTesting, type ShadowGate } from '../src/shadow-mode';
...
/**
 * Arm the shared shadow gate (and the shared selector engine) for a test.
 *
 * Must run BEFORE `plugin.setup()` but AFTER the plugin is constructed: the
 * plugin's DataExtractor captures the shared gate at construction time, so
 * resetting the gate here would hand the plugin a stale one. Use
 * {@link resetShadowState} in `afterEach` to get back to off.
 */
...
/**
 * Return to the off path. The gate is an arm-once latch scoped to a page, so a
 * config update alone cannot disarm it — tests need the explicit reset.
 */
...
/** A gate already armed at the given budget, for directly-constructed observables. */
...
describe('shadow-DOM end-to-end capture', () => {
  ...
  test('DISABLED: a click inside a shadow root retargets to the host and emits no delimiter', async () => {
    // Shadow mode off (the default). The event target is the host element.
    const config: Partial<BrowserConfig> = {
      defaultTracking: false,
      loggerProvider: loggerProvider as unknown as ILogger,
    };
    ...
    // The retargeted host (<my-host>) is not in the default allowlist, so no
    // trackable ancestor is found and typically nothing is emitted. Whatever is
    // emitted must carry no shadow delimiter, and must not throw.
...
describe('shadow-DOM exposure discovery', () => {
  ...
  test('ENABLED: observes an allowlisted element inside an open shadow root', () => {
    ...
    const obs = createExposureObservable(mutationObservable, ['.track-me'], armedGate(1));
  ...
  test('DISABLED: does NOT observe elements inside shadow roots (light-DOM only)', () => {
    const shadowButton = buildShadowExposure();
    const obs = createExposureObservable(mutationObservable, ['.track-me'], offGate());
    obs.subscribe(() => undefined);
