 ...
 * Every field is spelled out because `updateSelectorConfig` deliberately
 * ignores deliveries that carry neither switch — `undefined` would be a no-op,
 * not a reset.
 */
...
describe('shadow-DOM end-to-end capture', () => {
  ...
  test('DISABLED: a click inside a shadow root retargets to the host (no delimiter, byte-identical path)', async () => {
    ...
    // With shadow off, the retargeted host (<my-host>) matches no selector in
    // the default allowlist, so no trackable ancestor is found and NOTHING is
    // emitted — exactly the pre-shadow behavior. This is the off-path
    // invariant: a gate regression that pierces while disabled would emit an
    // Element Clicked here and fail this assertion.
