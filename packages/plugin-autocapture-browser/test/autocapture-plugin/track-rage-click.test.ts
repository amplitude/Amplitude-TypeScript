...
describe('trackRageClicks', () => {
  ...
  it('should work with zen-observable using ZenSubject helper', async () => {
    const subscription = trackRageClicks({
      amplitude: mockAmplitude,
      allObservables,
      shouldTrackRageClick,
    ...
    // Simulate clicks using the zen-observable
    const startTime = Date.now();
    for (let i = 0; i < DEFAULT_RAGE_CLICK_THRESHOLD; i++) {
      clickObserver.next({
        event: {
    ...
    // Advance timers to complete the timer
    jest.advanceTimersByTime(DEFAULT_RAGE_CLICK_WINDOW_MS + 100);

    // Run all pending timers and wait for async operations to complete
    await jest.runAllTimersAsync();
