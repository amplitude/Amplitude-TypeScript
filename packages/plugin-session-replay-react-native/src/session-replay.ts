  ...
  implements EnrichmentPlugin<ReactNativeClient, ReactNativeConfig>
{
  ...
  async setup(config: ReactNativeConfig, _: ReactNativeClient): Promise<void> {
    this.config = config;
    console.log(
      `Installing @amplitude/plugin-session-replay-react-native, version ${VERSION}.`
    );
  ...
  async execute(event: Event): Promise<Event | null> {
    ...
    if (
      ...
      this.config.sessionId !==
        (await PluginSessionReplayReactNative.getSessionId())
    ) {
    ...
    if (this.config.sessionId && this.config.sessionId === event.session_id) {
      const sessionRecordingProperties =
        await PluginSessionReplayReactNative.getSessionReplayProperties();
