...
const defaultElementClickedEvent = '[Amplitude] Element Clicked';

type Options = {
  restrictToAutocapture?: boolean;
};

export const pageUrlPreviousPagePlugin = ({ restrictToAutocapture = false }: Options): EnrichmentPlugin => {
  ...
  const getAdditionalEventProperties = (): AdditionalEventProperties => {
  ...
  const plugin: EnrichmentPlugin = {
    ...
    execute: async (event: Event) => {
      if (
        restrictToAutocapture &&
        (event.event_type === defaultPageViewEvent || event.event_type === defaultElementClickedEvent)
      ) {
        return event;
      }
