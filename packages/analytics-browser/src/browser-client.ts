...
export class AmplitudeBrowser extends AmplitudeCore implements BrowserClient, AnalyticsClient {
  ...
  protected async _init(options: BrowserOptions & { apiKey: string }) {
    ...
    const autocaptureAdapter = this.autocaptureAdapter();
    ...
    if (fetchRemoteConfig && autocaptureAdapter) {
      remoteConfigClient = await autocaptureAdapter.getRemoteConfigClient(options, loggerProvider, serverZone);
    ...
    /* istanbul ignore next */
    if (fetchRemoteConfig && remoteConfigClient && autocaptureAdapter) {
      await autocaptureAdapter.fetchRemoteConfig(remoteConfigClient, browserOptions);
    }
    ...
    await autocaptureAdapter?.addAttributionTrackingPlugin();
    ...
    await autocaptureAdapter?.addPlugins();
  ...
  setSessionId(sessionId: number | undefined) {
    ...
    const isCampaignEventTracked =
      this.autocaptureAdapter()?.trackCampaignEventIfNeeded(++lastEventId, promises) ?? false;
  ...
  async process(event: Event) {
    ...
    if (
      ...
      if (isEventInNewSession || shouldSetSessionIdOnNewCampaign) {
        ...
        this.autocaptureAdapter()?.trackCampaignEventIfNeeded();
      }
    }
  ...
  protected autocaptureAdapter(): AutocaptureAdapter | null {
    return {
      addPlugins: async () => {
