...
describe('migration', () => {
  ...
  const removeLegacyIdentify = jest.fn();
  const removeLegacyInterceptedIdentify = jest.fn();
  ...
  beforeEach(() => {
    ...
    removeLegacyIdentify.mockReset();
    removeLegacyInterceptedIdentify.mockReset();
  });
  ...
  test('should migrate legacy data', async () => {
    ...
    expect(removeLegacyIdentify.mock.calls).toEqual([
      ['test-instance', 4],
      ['test-instance', 5],
    ]);
    expect(removeLegacyInterceptedIdentify.mock.calls).toEqual([
      ['test-instance', 2],
      ['test-instance', 6],
    ]);
  });
