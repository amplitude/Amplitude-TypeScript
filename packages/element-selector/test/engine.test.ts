...
import { normalizeResolvedConfig, resolveSelectorConfig } from '../src/config/resolve-config';
...
describe('createSelectorEngine', () => {
  ...
  describe('getConfig', () => {
    it('returns the current resolved config', () => {
      ...
      expect(engine.getConfig()).toStrictEqual(normalizeResolvedConfig(initial));
    });
  });
  ...
  describe('updateConfig + onConfigChange', () => {
    it('replaces the active config and notifies all subscribers', () => {
      ...
      const normalized = normalizeResolvedConfig(next);
      engine.updateConfig(next);

      expect(cb1).toHaveBeenCalledWith(normalized);
      expect(cb2).toHaveBeenCalledWith(normalized);
      expect(engine.getConfig()).toStrictEqual(normalized);
    });
  ...
  describe('config compatibility', () => {
    it('applies shadow defaults when shadow fields are omitted from the resolved config', () => {
      setBody(`<button>x</button>`);
      ...
      expect(engine.getConfig().shadowDomEnabled).toBe(false);
      expect(engine.getConfig().maxShadowDomDepth).toBe(1);
      const target = document.querySelector('button') as Element;
      expect(engine.generate(target)).toBeTruthy();
    });
  });
});
