...
The only runtime dependency is `@amplitude/analytics-core` (for the shared logger interface) plus `tslib`.
...
When neither strategy produces a unique selector anywhere on the walk, the engine invokes `fallbackCssPath`. The fallback walks from the target back up to `<html>`, expressing each step as `tag:nth-of-type(n)` and terminating early at the first id that survives the autogenerated-id filter.
...
| `^[a-f0-9]{16,}$` | UUIDs without hyphens, build hashes | `a9b8c7d6e5f4321b` |
...
Every path that catches an error accepts an optional `ILogger` from `@amplitude/analytics-core`. When wired up, the engine surfaces:
