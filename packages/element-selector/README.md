...
| **Late `attachShadow`** | Roots attached to elements already in the DOM are not discovered by observers. Clicks still capture; mutation/exposure may miss. A global `attachShadow` patch is **not planned as a default fix** — session replay and other libs already patch the same API ([details](../plugin-autocapture-browser/SHADOW-DOM.md#monkey-patch-coexistence)). |
...
Pre-rollout validation: [`tools/shadow-harness/README.md`](../../tools/shadow-harness/README.md).
